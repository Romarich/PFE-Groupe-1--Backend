package pfe.backend.DAL.DAO;

import org.apache.tomcat.util.codec.binary.Base64;
import org.mindrot.jbcrypt.BCrypt;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.CommandLineRunner;
import org.springframework.boot.autoconfigure.condition.ConditionalOnProperty;
import org.springframework.stereotype.Component;
import pfe.backend.DAL.Models.Item;
import pfe.backend.DAL.Models.User;
import pfe.backend.DAL.Repositories.ExchangeRepository;
import pfe.backend.DAL.Repositories.ItemRepository;
import pfe.backend.DAL.Repositories.UserRepository;

/*CommandLineRunner makes it automatically execute by Spring when db is run*/
@Component
/*ConditionalOnProperty ne se lance que si le nom de la proprieté est à la value que j'ai indiqué.
 * C'est utile pour que la db ne soit pas effacé et remplie à chaque fois.*/
@ConditionalOnProperty(name = "pfe.db.recreate", havingValue = "true")
public class DbSeeder implements CommandLineRunner {
    @Autowired
    private UserRepository userRepository;
    @Autowired
    private ItemRepository itemRepository;
    @Autowired
    private ExchangeRepository exchangeRepository;

    @Override
    public void run(String... args) {
        User lissy = new User();
        lissy.setObjectID("1");
        lissy.setEmail("lissy@gmail.com");
        lissy.setPassword(BCrypt.hashpw("mdpLissy", BCrypt.gensalt()));
        lissy.setName("Carcani");
        lissy.setFirstname("Lissy");
        lissy.setAddress("Rue de lissy");
        lissy.setRole("ADMIN");
        lissy.setLogin("loginLi");
        lissy.setNbPoints(0);
        lissy.setBanned(false);

        Item sakuraCardCaptorT1 = new Item();
        sakuraCardCaptorT1.setObjectID("1");
        sakuraCardCaptorT1.setTitle("Sakura Card Captor T1");
        sakuraCardCaptorT1.setType("Book");
        sakuraCardCaptorT1.setDescription("The first volume of the Sakura Card Captor series");
        sakuraCardCaptorT1.setStock(200);
        sakuraCardCaptorT1.setValidate(true);
        byte[] imageByte= Base64.decodeBase64("");
        sakuraCardCaptorT1.setPicture(imageByte);

        Item kingomHearts3 = new Item();
        kingomHearts3.setObjectID("2");
        kingomHearts3.setTitle("Kingdom Hearts 3");
        kingomHearts3.setType("Game");
        kingomHearts3.setDescription("The last Kingdom Hearts entry to this day");
        kingomHearts3.setStock(100);
        kingomHearts3.setValidate(true);
        kingomHearts3.setPicture(imageByte);

        Item theIrishWayOfLiving = new Item();
        theIrishWayOfLiving.setObjectID("3");
        theIrishWayOfLiving.setTitle("The irish way of living");
        theIrishWayOfLiving.setType("Book");
        theIrishWayOfLiving.setDescription("Learn how to live like a true irish.");
        theIrishWayOfLiving.setStock(0);
        theIrishWayOfLiving.setValidate(true);
        theIrishWayOfLiving.setPicture(imageByte);

        /*Drop all users*/
        userRepository.deleteAll();
        /*Drop all items*/
        itemRepository.deleteAll();
        /*Drop all exchanges*/
        exchangeRepository.deleteAll();

        /*Add users to db*/

        userRepository.save(lissy);

        /*Add items to db*/
        itemRepository.save(sakuraCardCaptorT1);
        itemRepository.save(kingomHearts3);
        itemRepository.save(theIrishWayOfLiving);

    }
}
